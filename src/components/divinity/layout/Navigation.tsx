...
import { AppView } from '@/types/divinity';
import { Home, Headphones, MessageCircle } from 'lucide-react';
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 flex justify-center pointer-events-none font-sans">
      ...
      <div className="w-full bg-white/95 backdrop-blur-xl border-t border-white/20 shadow-[0_-8px_40px_rgba(0,0,0,0.08)] pb-[calc(1.5rem+env(safe-area-inset-bottom))] pt-3 px-12 flex justify-between items-center pointer-events-auto rounded-t-[32px]">
        ...
        >
          <div className={`transition-all duration-300 ${currentView === AppView.DASHBOARD ? 'text-[#FF8C69] scale-110' : 'text-slate-300'}`}>
            <Home size={26} strokeWidth={2.5} />
          </div>
          <span className={`text-[11px] font-bold uppercase tracking-[0.1em] ${currentView === AppView.DASHBOARD ? 'text-[#FF8C69]' : 'text-slate-400'}`}>
            Home
          </span>
        </button>
        ...
        >
          <div className="w-[68px] h-[68px] rounded-full bg-gradient-to-tr from-[#FF9B6A] to-[#EC4899] p-1 shadow-2xl shadow-orange-300/50">
        ...
        <button
          ...
          className="flex flex-col items-center gap-1.5 active:scale-90 transition-transform flex-1"
        >
          <div className={`transition-all duration-300 ${currentView === AppView.HEAL ? 'text-[#FF8C69] scale-110' : 'text-slate-300'}`}>
            <Headphones size={26} strokeWidth={2.5} />
          </div>
          <span className={`text-[11px] font-bold uppercase tracking-[0.1em] ${currentView === AppView.HEAL ? 'text-[#FF8C69]' : 'text-slate-400'}`}>
            Sound
          </span>
        </button>
