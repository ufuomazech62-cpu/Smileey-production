...
const ClippedButton = ({ children, variant = 'red', className = '' }: { children: React.ReactNode, variant?: 'red' | 'white', className?: string }) => (
  <button
    className={`px-6 py-3 font-bold uppercase tracking-tighter ${
      variant === 'red' ? 'bg-[#f5496c] text-white' : 'bg-white text-black'
    } ${className}`}
    style={{
  ...
  return (
    <div className="relative w-full h-screen bg-black overflow-hidden text-white">
      ...
      <div className="absolute bottom-8 left-8 md:left-16 z-10 p-6 rounded-xl border border-white/12 backdrop-blur-[40px] backdrop-saturate-[180%] bg-white/5 shadow-inner">
        ...
        <ClippedButton variant="red" className="font-display">Get Started</ClippedButton>
      </div>
    </div>
  );
}
